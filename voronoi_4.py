...
class VoronoiPlot:
    @_decorator
    def _held_figure(self, func, obj, ax=None, **kw):
...
#     #vor is instance of module Volonoi 
#     #points : coordinates of points to construct a Voronoi diagram from
#         if vor.points.shape[1] != 2:
#             raise ValueError("Voronoi diagram is not 2-D")
#     # show_points : add the Voronoi vertices to the plot
#     # point_size : specifies the size of points
#     # if show_points is empty, return true and if sentence start
#         if kw.get('show_points', True):
#         # if point_size is empty, return none
#             point_size = kw.get('point_size', None)
#             ax.plot(vor.points[:, 0], vor.points[:, 1], '.', markersize=point_size)
...
#     # print(finite_segments)      #セルの境界線の組
...
class Voro:
    ...
    def draw_voronoi(self, ax):
        ...
        v = VoronoiPlot()
        v.voronoi_plot(vor, ax=ax)

        # print(vor.ridge_points)
        # print(vor.points.shape[1])
